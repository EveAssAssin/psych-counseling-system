...
2. **交叉驗證**：工單問題 + LINE 訊息 + 面談內容是否呈現一致的訊號
...
`;
    ...
    if (data.reviews.length > 0) {
      ...
      input += `【評價/客訴統計（近 30 天）】
正面：${rs.positive}｜負面：${rs.negative}｜待處理：${rs.pending}${rs.pending > 0 ? ' ⚠️' : ''}
平均回覆速度：${rs.avg_response_hours > 0 ? rs.avg_response_hours + ' 小時' : '無資料'}
...
`;
    ...
    // ── 資料摘要 ──
    input += `【資料摘要】
LINE 訊息：${data.officialMessages.length} 筆 | 面談：${data.conversations.length} 筆 | 工單：${data.tickets.length} 筆 | 評價：${data.reviews.length} 筆
`;
