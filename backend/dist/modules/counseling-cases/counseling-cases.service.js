...
var CounselingCasesService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.CounselingCasesService = void 0;
...
const lefthand_api_service_1 = require("../sync/lefthand-api.service");
let CounselingCasesService = CounselingCasesService_1 = class CounselingCasesService {
    constructor(supabase, insight, holidays, draftStore, planner, lefthand) {
        ...
        this.logger = new common_1.Logger(CounselingCasesService_1.name);
    }
    get db() {
    ...
    async getEmployeeAttendance(appNumber, startDate, endDate) {
        ...
        const { data: emp, error } = await this.db
            ...
            .single();
        if (error || !emp) {
            throw new common_1.NotFoundException(`找不到員工 ${appNumber}`);
        }
        ...
        const empData = (r.data || []).find((d) => d.employeeErpid === emp.employeeerpid) || r.data?.[0];
    ...
    async listCases(query) {
        ...
        if (query.supervisor_id)
            q = q.eq('supervisor_id', query.supervisor_id);
        if (query.employee_app_number)
            q = q.eq('employee_app_number', query.employee_app_number);
        if (query.state_tag_code)
            q = q.contains('state_tag_codes', [query.state_tag_code]);
    ...
    async getCase(id) {
        ...
        if (error || !caseRow)
            throw new common_1.NotFoundException(`Case ${id} not found`);
    ...
    async closeCase(id, closingSummary) {
    ...
    async createDraft(dto) {
        const { data: employee, error: empErr } = await this.db
            ...
            .single();
        if (empErr || !employee) {
            throw new common_1.NotFoundException(`找不到員工 ${dto.employee_app_number}`);
        }
        ...
        if (supErr || !supervisor) {
            throw new common_1.NotFoundException(`找不到輔導員 ${dto.supervisor_id}`);
        }
        if (!supervisor.is_active) {
            throw new common_1.BadRequestException('輔導員帳號已停用');
        }
    ...
    async confirmCase(dto) {
        const draft = this.draftStore.get(dto.draft_token);
        if (!draft) {
            throw new common_1.BadRequestException('草稿已過期或不存在，請重新生成');
        }
        ...
        if (dto.adjusted_plan_items && dto.adjusted_plan_items.length > 0) {
            ...
            finalItems = dto.adjusted_plan_items
                ...
                if (!allowed.has(it.method)) {
                    throw new common_1.BadRequestException(`第 ${idx + 1} 項使用了未授權的方法：${it.method}`);
                }
                return {
        ...
        if (finalItems.length === 0) {
            throw new common_1.BadRequestException('至少需要 1 個排程節點');
        }
    ...
    async updatePlanItem(itemId, dto) {
        if (dto.scheduled_date) {
            ...
            if (existing && !existing.original_scheduled_date) {
                dto.original_scheduled_date = existing.scheduled_date;
            }
    ...
    async createExecution(caseId, dto) {
        const payload = {
            case_id: caseId,
    ...
    async getOverdueTasks(supervisorId) {
...
exports.CounselingCasesService = CounselingCasesService;
exports.CounselingCasesService = CounselingCasesService = CounselingCasesService_1 = __decorate([
...
//# sourceMappingURL=counseling-cases.service.js.map